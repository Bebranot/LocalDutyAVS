...
# КПК, панель призрака, борга и ИИ строят этот ключ сами из имени уровня. Без него они
# показали бы игроку сырой "alert-level-alpha-instructions" вместо текста.
alert-level-alpha-instructions = Ограничения доступа сняты со всей станции. Вооружитесь, заприте шлюзы и держитесь группами. Помощь не придёт вовремя.
...
duty-code-alpha-access-no-card = Аварийный режим доступа привязан к ID-карте, а её при вас нет. Найдите карту — свою или чужую.
